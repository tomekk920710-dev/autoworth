...
const app=express(), PORT=process.env.PORT||3000, JWT_SECRET=process.env.JWT_SECRET||"dev-only-change-me";
...
app.use(express.json({limit:"100kb"})); app.use(cookieParser()); app.get("/",(req,res)=>res.sendFile(path.join(__dirname,"index.html")));
...
CONFIDENCE CALIBRATION: confidence is a 0-100 percentage, NOT a 0-10 score. Use 80-100 when the workflow is concrete and the steps are clear; 60-79 when some important details are missing; 40-59 when several assumptions are required; below 40 only when the workflow is genuinely ambiguous. Do not use 1-10 style numbers such as 1, 5 or 10 unless the interpretation is genuinely almost impossible.
AUTOMATION POTENTIAL: use 0-100 as the percentage of that specific step that could realistically be automated under the recommended mode. Repeatable structured low-risk steps can be high; judgement-heavy or high-stakes steps must be low and KEEP_HUMAN. Do not use 1% as a generic placeholder.
...
app.get("/api/billing/checkout-status",auth,async(req,res)=>{
  ...
  try{
    ...
    res.json({status:s.status,paymentStatus:s.payment_status,paid:s.payment_status==="paid"});
  }catch(e){res.status(400).json({error:"Could not verify checkout session."})}
});
...
app.post("/api/analyze",async(req,res)=>{const workflow=String(req.body.workflow||"").trim();if(workflow.length<20)return res.status(400).json({error:"Please describe a real workflow in a few sentences."});if(workflow.length>12000)return res.status(400).json({error:"Workflow is too long."});if(!process.env.OPENAI_API_KEY)return res.json({source:"fallback",analysis:fallback(workflow)});try{const client=new OpenAI({apiKey:process.env.OPENAI_API_KEY});const r=await client.responses.create({model:MODEL,instructions,input:workflow,store:false,reasoning:{effort:"none"},temperature:.2,text:{format:{type:"json_schema",name:"autoworth_workflow",strict:true,schema},verbosity:"low"}});res.json({source:"openai",analysis:JSON.parse(r.output_text),usage:r.usage||null})}catch(e){console.error(e);res.status(502).json({error:"AI analysis failed. Please try again."})}});
...
app.post("/api/reports",auth,(req,res)=>{
 ...
 if(!workflow||!analysis||!economics)return res.status(400).json({error:"Missing report data"});
 const u=db.prepare("SELECT plan,report_credits FROM users WHERE id=?").get(req.user.id);
 if(u.plan!=="pro" && u.report_credits<=0)return res.status(402).json({error:"Full Report unlock required.",code:"PAYWALL"});
 if(u.plan!=="pro")db.prepare("UPDATE users SET report_credits=report_credits-1 WHERE id=?").run(req.user.id);
 const r=q.insertReport.run(req.user.id,title,workflow,JSON.stringify(analysis),JSON.stringify(economics));
...
app.listen(PORT,()=>console.log("AutoWorth V1.2 on http://localhost:"+PORT));
